...
Amperize.prototype.parse = function parse(content, callback) {
    ...
    return new Promise(function(resolve, reject) {
        var id = uuid.v4();
        _this.once('read', _this.amperizer.bind(_this, id));
        _this.once('parsed: ' + id, callback);
        resolve(_this.htmlParser.parseComplete(content));
    });
};
...
Amperize.prototype.traverse = async function traverse(data, html, done) {
    ...
    async.reduce(data, html, function reduce(html, element, step) {
        ...
        if (/(style|script|textarea)/.test(element.name)) {
            element = null;
        }
